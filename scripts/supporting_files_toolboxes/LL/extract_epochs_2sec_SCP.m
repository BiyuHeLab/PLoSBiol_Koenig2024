...
function [TS1m,fintable] = extract_epochs_2sec_052421_SCP(subject,catseq,window,band_name,path_base)
...
if window==1
    after = 1:3001; %changed from 1:2401 on 3/31/21 by LK - intending to obtain -2 to + 3 sec around stim presentation -- need to check this is what this does
else
...
% % %         megpost '.mat']);
    ...
    %load processed MEG data
    M = load([si.path_ica band_name '/data_clean_segmented_0' int2str(megtest(j))  '.mat']);
