...
for timepoint = 1:4; 
    ...
    plot_option=1; %plotting not working - check why
    clusters_orig{timepoint} = find_clusters_LK(topo_stat, topo_p, p_thresh, plot_option, si);
    ...
    for i_rep = 1:nReps
        ...
        for i_sensor = 1:nSensors
            ...
            for i_sub = 1:nSubs
                ...
                seen_shuffle(i_sub) = mean(dv_orig{i_sub}{timepoint}(i_sensor, ind(1:cond_n{i_sub}(1))));
                unseen_shuffle(i_sub) = mean(dv_orig{i_sub}{timepoint}(i_sensor, ind((cond_n{i_sub}(1)+1):(cond_n{i_sub}(1)+cond_n{i_sub}(2)))));
            end
        ...
        clusters_shuffle{i_rep} = find_clusters_LK(topo_stat_shuffle, topo_p_shuffle, p_thresh);
...
save([data_dir, '../data/betaclusters.mat'],'clusters_orig');
